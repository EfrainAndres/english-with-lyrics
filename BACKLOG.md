...
> Production controlled-test state: exactly one isolated Email 1 remains attached to exactly one matching **STAGED / INACTIVE** Production workflow. One authorized affirmative submission produced one assignment, one workflow entry and one automated Email 1; approved content, CTA, unsubscribe and cleanup passed, while the hosted form redirect failed because the provider-native success state rendered instead of `/gracias`. The group returned to zero; the workflow retains zero in progress and one completed history record. No public-site link, Vercel value, deployment, test send, campaign send, transactional send or additional message was created. Production consent linking, corrective retest, controlled rollout and indexing remain **PENDING**; lead capture and email delivery remain **INACTIVE**; direct PDF delivery remains **ACTIVE**.
...
- [ ] Correct the hosted-form redirect without submission, then obtain separate authorization for exactly one corrective retest.
