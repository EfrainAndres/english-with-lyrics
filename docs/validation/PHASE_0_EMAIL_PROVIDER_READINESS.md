...
Production provider-hosted consent staging: STAGED / INACTIVE from the website perspective. Exactly one isolated Production group and one matching provider-hosted form remain without changes to existing Preview or unrelated Production resources. The group has zero subscribers after controlled cleanup; the form retains one non-personal controlled registration. The form requires email, keeps name optional, uses the approved consent wording unchanged in one unchecked affirmative control, links to the exact `/privacidad` destination, and uses provider-native form/source and copy-version identity while MailerLite remains responsible for capture channel/time, subscriber status, and unsubscribe/suppression state.

The staged form has double opt-in disabled and single opt-in persisted after reload. Its one controlled submission verified consent blocking and the affirmative path, but the form redirect FAILED because the provider-native success state rendered instead of `/gracias`. That failed result remains historical evidence. The form success action is now configured for the approved `/gracias` redirect and was reverified without submission; it is STAGED / NOT YET REVALIDATED BY SUBMISSION. It has no campaign, public-site link, Vercel configuration or Production deployment. MailerLite offers no native unpublish/deactivate state for this hosted form; possible reachability through its undistributed direct provider URL is an owner-accepted staging limitation only and does not authorize public linking. The known MailerLite-native missing-consent feedback limitation remains tracked and is not fixed, certified, or approved for public activation. Final Production consent linking remains PENDING; the Production group/form is STAGED / INACTIVE from the website perspective; lead capture, Email 1 delivery and automation remain INACTIVE; corrective retest, controlled rollout and indexing remain PENDING; direct PDF delivery remains ACTIVE.
...
- Technical: reconfirm the staged hosted-form redirect configuration without submission before the corrective test window. It is configured and verified without submission, but not yet revalidated by submission.
- Owner authorization: the completed one-contact authorization is consumed; separately approve one corrective retest window, temporary automation activation and cleanup.
- Controlled test: before public promotion, complete a separately authorized one-contact retest and require the provider-hosted form itself to redirect to `/gracias`; the delivered-email CTA PASS does not satisfy this failed form gate.
...
Reason:
The provider-hosted Preview workflow, published legal routes, professional/provider reviews, existing verified sender, `/gracias`, rights-request QA and direct PDF path have passed their recorded checks. The controlled Production test passed consent, assignment, one automated delivery, received content, CTA, unsubscribe, cleanup and isolation checks, but FAILED because the provider-hosted form did not redirect to `/gracias`; that failure remains historical evidence. The form success action is now configured for the approved redirect and verified without submission, but it is not yet revalidated by submission. The staged Production group is empty after cleanup; the inactive workflow retains zero contacts in progress and one completed history record, and the form retains one non-personal registration. Production website support remains unconfigured and fail-closed. A corrective retest, activation acceptance for the provider-native limitations, analytics-dashboard verification, indexing and the 10-subscriber/72-hour rollout remain separate gates. Direct PDF delivery remains active and independent.
