...
Controlled Production funnel test: **FAIL / NO-GO FOR PUBLIC ACTIVATION**. One
...
The staged form has double opt-in disabled and single opt-in persisted after reload. Its first controlled submission verified consent blocking and the affirmative path, but the form redirect FAILED because the provider-native success state rendered instead of `/gracias`; that result remains historical evidence. The corrected success action subsequently redirected to `/gracias` in two separately authorized affirmative submissions, including a fresh-contact attempt with zero prior subscriber or workflow history. Both created the intended isolated group assignment but zero new workflow entries and zero new Email 1 deliveries. The form has no campaign, public-site link, Vercel configuration or Production deployment. MailerLite offers no native unpublish/deactivate state for this hosted form; possible reachability through its undistributed direct provider URL is an owner-accepted staging limitation only and does not authorize public linking. The known MailerLite-native missing-consent feedback limitation remains tracked and is not fixed, certified, or approved for public activation. Final Production consent linking remains PENDING; the Production group/form is STAGED / INACTIVE from the website perspective; lead capture, Email 1 delivery and automation remain INACTIVE; provider-level trigger diagnosis, any further retest, controlled rollout and indexing remain PENDING; direct PDF delivery remains ACTIVE.

Production Email 1 and automation state: STAGED / INACTIVE after controlled testing. Exactly one Email 1 remains attached to exactly one matching inactive Production workflow. The workflow uses only the original staged Production group/form path and now records zero contacts in progress and one completed non-personal history record. The approved Spanish subject, preheader, body, Production-scoped CTA, text-compatible alternative, provider privacy/footer and unsubscribe treatment were verified in exactly one automated controlled delivery with the existing verified sender unchanged.

The controlled test created one temporary group assignment, one workflow entry and one automated Production Email 1 delivery, then completed real unsubscribe and GDPR Forget cleanup. No segment, campaign audience, test send, campaign send, transactional send, public-site link, Vercel value or deployment was created; sent-campaign and outbox counts remain zero, and no additional regular campaign draft was added. Existing Preview resources were unchanged. The known missing-consent feedback and undistributed direct-provider-reachability limitations remain tracked and do not authorize public activation. Final Production consent linking, corrective retest, controlled rollout and indexing remain PENDING; lead capture, Email 1 delivery and automation remain INACTIVE; direct PDF delivery remains ACTIVE.

Fresh-contact corrective retest: **FAIL / NO-GO FOR PUBLIC ACTIVATION**. Exact-contact preflight returned zero subscriber records and zero active/completed workflow history. One authorized affirmative submission redirected to `/gracias` and created one isolated Production group assignment while the workflow was active. The workflow nevertheless created zero new entries and delivered zero Email 1 messages. No CAPTCHA, confirmation, verification, test, campaign, transactional or additional message occurred. The workflow was paused; manual unsubscribe required no email; GDPR Forget returned contact search and the group to zero. Post-test workflow state is zero in progress and one retained completed history record. Delivered content, CTA, footer/privacy and email-link unsubscribe behavior are UNVERIFIED for this attempt because no message existed. Direct PDF and legal routes pass independently; public Production remains fail-closed and Vercel, source, deployment, indexing and Preview remain unchanged.
...
- Active Production Email 1 delivery, Production automation, Production lead capture and controlled rollout remain pending/inactive; one controlled automated Email 1 delivery is retained as test evidence.
...
- Technical: preserve the now-passing hosted-form redirect and diagnose why the exact isolated group-join event produced zero new workflow entries in both post-correction attempts, including the fresh-contact attempt with no prior history. Do not submit or send during diagnosis.
- Owner authorization: all completed one-contact authorizations are consumed. Any further submission, temporary automation activation, message delivery or cleanup requires new explicit authorization after a minimum provider-level correction is identified.
- Controlled test: before public promotion, complete a newly authorized end-to-end retest only after the trigger cause is understood; require one assignment, one workflow entry and exactly one Email 1 in addition to the already-passing redirect.
...
Reason:
The provider-hosted Preview workflow, published legal routes, professional/provider reviews, existing verified sender, `/gracias`, rights-request QA and direct PDF path have passed their recorded checks. The first controlled Production test passed assignment and one automated delivery but failed the form redirect; that failure remains historical evidence. The corrected redirect now passes, including for a fresh contact with zero prior subscriber or workflow history, but both post-correction assignments created zero new workflow entries and zero Email 1 deliveries. The latest cleanup returned contact search and the staged Production group to zero; the inactive workflow retains zero contacts in progress and its original one completed history record. Production website support remains unconfigured and fail-closed. Provider-level trigger diagnosis, a newly authorized end-to-end retest, activation acceptance for the provider-native limitations, analytics-dashboard verification, indexing and the 10-subscriber/72-hour rollout remain separate gates. Direct PDF delivery remains active and independent.
