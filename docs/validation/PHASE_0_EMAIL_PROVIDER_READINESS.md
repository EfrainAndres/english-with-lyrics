...
The staged form has double opt-in disabled and single opt-in persisted after reload. Its first controlled submission verified consent blocking and the affirmative path, but the form redirect FAILED because the provider-native success state rendered instead of `/gracias`; that result remains historical evidence. The corrected success action subsequently redirected to `/gracias` in two separately authorized affirmative submissions, including a fresh-contact attempt with zero prior subscriber or workflow history. Both created the intended isolated group assignment. The reused contact did not re-enter; the fresh-contact window initially showed zero new workflow entries and sends, but later read-only provider evidence showed that contact in Completed with one automated send recorded. The authorized inbox contains zero matching messages and zero unexpected additional messages, so received delivery and content/CTA evidence remain UNVERIFIED. The form has no campaign, public-site link, Vercel configuration or Production deployment. MailerLite offers no native unpublish/deactivate state for this hosted form; possible reachability through its undistributed direct provider URL is an owner-accepted staging limitation only and does not authorize public linking. The known MailerLite-native missing-consent feedback limitation remains tracked and is not fixed, certified, or approved for public activation. Final Production consent linking remains PENDING; the Production group/form is STAGED / INACTIVE from the website perspective; lead capture, Email 1 delivery and automation remain INACTIVE; any further retest, controlled rollout and indexing remain PENDING; direct PDF delivery remains ACTIVE.

Production Email 1 and automation state: STAGED / INACTIVE after controlled testing. Exactly one Email 1 remains attached to exactly one matching inactive Production workflow. The workflow uses only the original staged Production group/form path and now records zero contacts in progress and two completed non-personal history records. MailerLite records two automated controlled sends, while exactly one historical automated delivery has inbox-verified approved Spanish subject, preheader, body, Production-scoped CTA, text-compatible alternative, provider privacy/footer and unsubscribe treatment with the existing verified sender unchanged. The fresh-contact provider-recorded send has no matching inbox message, so none of those received-message checks is credited to it.
...
- Active Production Email 1 delivery, Production automation, Production lead capture and controlled rollout remain pending/inactive; one historical controlled automated Email 1 delivery is inbox-verified and one additional provider send is recorded without matching inbox evidence.
...
- Owner authorization: all completed one-contact authorizations are consumed. Any further submission, temporary automation activation, exactly one automated Email 1 or cleanup requires new explicit authorization.
- Controlled test: before public promotion, complete one newly authorized end-to-end retest with a fresh contact; require one assignment, one terminal workflow entry, exactly one inbox-located Email 1 and complete received-content/CTA evidence in addition to the already-passing redirect.
...
Reason:
The provider-hosted Preview workflow, published legal routes, professional/provider reviews, existing verified sender, `/gracias`, rights-request QA and direct PDF path have passed their recorded checks. The first controlled Production test passed assignment and one inbox-verified automated delivery but failed the form redirect; that failure remains historical evidence. The corrected redirect now passes, including for a fresh contact with zero prior subscriber or workflow history. The fresh attempt's initial zero-entry/zero-send observation was a bounded-window snapshot: later provider evidence showed a second Completed entry and one provider-recorded send, but the authorized inbox contains no matching message. The latest cleanup returned contact search and the staged Production group to zero; the inactive workflow retains zero contacts in progress and two completed history records. Production website support remains unconfigured and fail-closed. A newly authorized end-to-end inbox-verification retest, activation acceptance for the provider-native limitations, analytics-dashboard verification, indexing and the 10-subscriber/72-hour rollout remain separate gates. Direct PDF delivery remains active and independent.
