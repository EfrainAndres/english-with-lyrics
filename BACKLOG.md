...
> Production controlled-test state: exactly one isolated Email 1 remains attached to exactly one matching **STAGED / INACTIVE** Production workflow. One authorized affirmative submission produced one assignment, one workflow entry and one automated Email 1; approved content, CTA, unsubscribe and cleanup passed, while the hosted form redirect failed because the provider-native success state rendered instead of `/gracias`. That failed test remains historical evidence. The form success action is now corrected and verified without submission, but is **STAGED / NOT YET REVALIDATED BY SUBMISSION**. The group remains at zero; the workflow retains zero in progress and one completed history record. No public-site link, Vercel value, deployment, new submission, test send, campaign send, transactional send or additional message was created. Production consent linking, corrective retest, controlled rollout and indexing remain **PENDING**; lead capture and email delivery remain **INACTIVE**; direct PDF delivery remains **ACTIVE**.
...
- [ ] Complete a passing controlled Production test before public promotion. The first authorized test passed assignment, one Email 1, delivered-email CTA, direct PDF independence, unsubscribe, cleanup and isolation, but FAILED because the provider-hosted form did not redirect to `/gracias`.
- [x] Correct and verify the hosted-form redirect configuration without submission. Result: STAGED / NOT YET REVALIDATED BY SUBMISSION; the earlier controlled-test failure remains historical evidence.
- [ ] Obtain separate authorization for exactly one corrective one-contact retest and require the provider-hosted redirect itself to pass before public promotion.
