...
- Approved the production origin (`https://english-with-lyrics.vercel.app`), restored the absolute `/ebook-gratis` backlink inside the public PDF, regenerated and metadata-patched the 21-page artifact (SHA-256 `c5ae8fba...`), and validated seven intended PDF link annotations with no local or preview URLs. Added `productionOrigin` and `ebookPageUrl` constants to `src/lib/links.ts`. Updated export script to require exactly 7 annotations and reject non-production vercel.app URLs.
- Aligned the Phase 0 home landing page with the active ebook delivery flow: routed all ebook CTAs to `/ebook-gratis` (direct download page) instead of the Tally ebook interest form, replaced waiting-state copy with published-guide copy ("Guía gratuita disponible", "Aprende inglés con 3 canciones"), updated the ebook section content list from guide-path options to actual guide contents (3 canciones, 9 prácticas, significados y vocabulario, ejercicios y reto), reframed the waitlist section as "Sigue aprendiendo con nosotros" with only survey and first-group CTAs, and added `ebookPagePath = "/ebook-gratis"` to `src/lib/links.ts`. Validation: `npm run lint` 0 errors, `npm run build` passes; documented in `docs/validation/PHASE_0_HOME_EBOOK_DELIVERY_ALIGNMENT.md`.
- Added the complete Phase 0 ebook production layout and validated review PDF, with nine complete learning blocks, dedicated song-review pages, reproducible export tooling, and structural PDF QA documentation: 21-page production HTML (`docs/design/production/phase-0-ebook.html`) with all 9 approved educational fragments, dedicated quick-review pages for each of the three songs (pages 8, 13, 18), production CSS (`docs/design/production/phase-0-ebook.css`), export script (`scripts/export-ebook-pdf.sh`) with server-readiness loop and structural PDF validation, QA checklist (`docs/validation/PHASE_0_EBOOK_PDF_QA.md`), and a validated draft PDF (1,223,965 bytes, 21 pages). Completed final PDF production QA corrections, including full TR-03 content visibility and prevention of localhost link targets in exported PDFs. Added the Phase 0 ebook publication-readiness review covering visual QA, link annotations, metadata, legal-safe publication checks, filename approval, public-origin readiness, delivery blockers, and go/no-go criteria. Publication-readiness decision: CONDITIONAL GO — content and link QA complete; blocked on owner visual sign-off and production domain approval. Enabled direct Phase 0 ebook delivery through `/ebook-gratis`, added the approved 21-page PDF under `public/downloads`, corrected PDF metadata (Author, Subject, Keywords, Creator) via stdlib-only incremental update, documented source-to-public integrity and delivery QA, and preserved survey and first-group follow-up actions. Deployed smoke test and production-domain confirmation remain pending.
- Applied visual QA corrections to the Phase 0 ebook PDF prototype: increased cover logo from 36px to 54px, added `.page--challenge` modifier to fit the three-day challenge in A5 with tighter spacing, replaced `·` dot markers in the confidence table with `○` circles styled at 1rem for clarity on paper, replaced the `a[href]::after` global URL-expansion approach with explicit `.print-url` spans to prevent full YouTube watch URLs and Tally query strings from duplicating beside link labels in print, and changed `.page` screen overflow from hidden to visible so long fragment cards surface instead of clipping silently.
