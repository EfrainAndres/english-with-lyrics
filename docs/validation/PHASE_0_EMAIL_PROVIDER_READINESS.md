...
- Technical: correct and verify the hosted-form redirect mapping without a submission before any corrective retest.
...
Reason:
The provider-hosted Preview workflow, published legal routes, professional/provider reviews, existing verified sender, `/gracias`, rights-request QA and direct PDF path have passed their recorded checks. The controlled Production test passed consent, assignment, one automated delivery, received content, CTA, unsubscribe, cleanup and isolation checks, but FAILED because the provider-hosted form did not redirect to `/gracias`. The staged Production group is empty after cleanup; the inactive workflow retains zero contacts in progress and one completed history record, and the form retains one non-personal registration. Production website support remains unconfigured and fail-closed. A corrective retest, activation acceptance for the provider-native limitations, analytics-dashboard verification, indexing and the 10-subscriber/72-hour rollout remain separate gates. Direct PDF delivery remains active and independent.
