...
> Production consent state: exactly one isolated MailerLite Production group and one matching provider-hosted consent form remain **STAGED / INACTIVE from the website perspective** with zero group subscribers after controlled cleanup, single opt-in configured, no public-site or Vercel link, and retained non-personal controlled registration history. MailerLite offers no native unpublish/deactivate state for this hosted form, so direct provider-URL reachability is an owner-accepted staging limitation; the URL is not distributed. The known missing-consent feedback limitation remains a tracked pre-activation follow-up. Production consent linking remains **PENDING** and direct PDF delivery remains **ACTIVE**.

> Production controlled-test state: exactly one isolated Email 1 remains attached to exactly one matching **STAGED / INACTIVE** Production workflow. The first test produced one workflow entry and one inbox-verified automated Email 1 but failed redirect. Two later submissions passed redirect and assignment; the first reused a prior identity and remained without a new entry. During the fresh-contact window, workflow and send counters remained zero through the bounded wait and cleanup. Read-only follow-up later showed that the fresh contact had reached Completed and that MailerLite recorded one automated send, superseding the zero-entry/zero-send snapshot. The authorized inbox contains zero messages for that fresh contact, so delayed inbox delivery, rendered content, CTA, footer/privacy and unsubscribe rendering remain UNVERIFIED; unexpected additional inbox messages are also zero. Provider completion/send visibility occurred only after cleanup, but the evidence does not establish when execution occurred. The group and contact search remain zero; the workflow is paused with zero in progress and two completed history records. Production consent linking, another controlled retest, rollout and indexing remain **PENDING**; lead capture and active email delivery remain **INACTIVE**; direct PDF delivery remains **ACTIVE**.
...
- [ ] Complete a passing controlled Production test before public promotion. The first test failed redirect; the later fresh-contact attempt eventually reached Completed with one provider-recorded send, but its inbox delivery and received content/CTA evidence remain UNVERIFIED.
...
- [ ] Obtain separate owner authorization for one new fresh-contact retest and exactly one automated Email 1 because delayed inbox content/CTA evidence remains missing.
